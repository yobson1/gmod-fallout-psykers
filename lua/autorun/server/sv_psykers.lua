	...
	["body"] = {
		Activate = function(ply)
			...
			if IsValid(target) and target:IsPlayer() then
				jlib.Possess(ply, target)
				target:ScreenFade(SCREENFADE.IN, Color(255, 255, 255, 200), 0.8, 0)
				jlib.SendSound("psykers/ui_wildwasteland.ogg", target)
				return Psykers.Config.Cooldowns.Body
			else
				ply:falloutNotify("Invalid target")
				return 5
			end
		end,
		Deactivate = function(ply)
...
function Psykers.IsAnyAbilityAvailable()
	for abilityID, ability in ipairs(Psykers.Abilities) do
